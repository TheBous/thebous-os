...
| Review | `thebous-os:review-pr-multiharness` | Default structured review. Use `review-pr-multiharness-ponytail` when a deletion pass is required |
...
- Reviewers use `thebous-os:review-pr-multiharness` by default. When a deletion
  pass is required, use `thebous-os:review-pr-multiharness-ponytail` instead — it
  already runs the parent workflow. Do not run both. Authors resolve comments with
